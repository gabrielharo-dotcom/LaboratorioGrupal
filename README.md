## Evidencias del Laboratorio Desarrollado

### Integrante: Haro Vargas Gabriel
1.- Crear Proyecto
![1](Docs/Evidencia1-Haro.png)

2.- Limpiar Proyecto
![2](Docs/Evidencia2-Haro.png)

3.- Instalar Tailwind
![3](Docs/Evidencia3-Haro.png)

4.- Configurar Alias
![4](Docs/Evidencia4-Haro.png)

5.- Configurar e Instalar Shadcn
![5](Docs/Evidencia5-Haro.png)

6.- Instalar y configurar axios
![6](Docs/Evidencia6-Haro.png)

7.- Renderizado de información
![7](Docs/Evidencia7-Haro.png)


### Integrante: Olano Paniora Oscar

1.- Crear Proyecto
![1](docsOscarOlano/crear proyecto-OSCAR.png)

2.- Limpiar Proyecto
![2](docsOscarOlano/limpiar proyecto-OSCAR.png)

3.- Instalar Tailwind
![3](docsOscarOlano/instalar tailwind-OSCAR.png)

4.- Configurar Alias
![4](docsOscarOlano/configurar alias-OSCAR.png)

5.- Configurar e Instalar Shadcn
![5](docsOscarOlano/configurar e instalar Shadcn-OSCAR.png)

6.- Instalar y configurar axios
![6](docsOscarOlano/Instalar y configurar axios-OSCAR.png)

7.- Renderizado de información
![7](docsOscarOlano/Renderizado e informacion-OSCAR.png)



### Integrante: Quintana Llanque Rony

1.- Crear Proyecto
![1](docsRonyQuintana/primeraevidencia.png)

2.- Limpiar Proyecto
![2](docsRonyQuintana/SegundaEvidencia.png)

3.- Instalar Tailwind
![3](docsRonyQuintana/TerceraEvidencia.png)

4.- Configurar Alias
![4](docsRonyQuintana/cuartaevidencia.png)

5.- Configurar e Instalar Shadcn
![5](docsRonyQuintana/Quintaevidencia.png)

6.- Instalar y configurar axios
![6](docsRonyQuintana/SextaEvidencia.png)

7.- Renderizado de información
![7](docsRonyQuintana/septimaevidencia.png)
